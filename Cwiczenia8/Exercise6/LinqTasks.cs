...
namespace Exercise6
{
    public static class LinqTasks
    {
        ...
        {
            IEnumerable<object> result =
                (from e in Emps select new { e.Ename, e.Job, e.HireDate })
                .Union
        ...
        public static IEnumerable<object> Task11()
        {
            IEnumerable<object> result = 
                    ...
                    join d in Depts on g.Key equals d.Deptno
                    where g.Count() > 1
                    select new {Name = d.Dname, NumOfEmployees = g.Count()};;
            return result;
        }
        ...
        {
            IEnumerable<Emp> result = Emps.Employees() ;
            return result;
        }
        ...
        {
            int result = arr.GroupBy(x=> x).Single(t => t.Count() %2 !=0).Key;
            //result=
            return result;
        }
        ...
        public static IEnumerable<Dept> Task14()
        {
            IEnumerable<Dept> result = 
                    from d in Depts
                    join e in Emps on d.Deptno equals e.Deptno into deptGroup
                    let count = deptGroup.Count()
                    where count == 5 || count == 0
                    orderby d.Dname
                    select d;
    ...
    {
        public static IEnumerable<Emp> Employees(this IEnumerable<Emp> emp)
        {
            IEnumerable<Emp> result =
                from e in emp
                where emp.Any(y => y.Mgr != null && y.Mgr.Empno == e.Empno)      
